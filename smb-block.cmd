...
for /f %%f in (internet.txt) do (
	...
	netsh advfirewall firewall add rule name="Block SMB/NBSS to Internet - %%f" dir=out action=block enable=yes Profile=any Localip=any Remoteip=%%f Protocol=tcp Interfacetype=any remoteport=445,139
	netsh advfirewall firewall add rule name="Block NBDS/NBNR to Internet - %%f" dir=out action=block enable=yes Profile=any Localip=any Remoteip=%%f Protocol=udp Interfacetype=any remoteport=137,138
)
echo [+] read %count% IPs
